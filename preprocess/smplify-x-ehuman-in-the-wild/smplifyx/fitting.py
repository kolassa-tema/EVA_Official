...
class FittingMonitor(object):
    ...
    def create_fitting_closure(self,
                               ...
                               gt_joints=None, loss=None, p3DGT_hand=None, psmplx_bodyGT=None,
                               joints_conf=None,
                               joint_weights=None,
        ...
        def fitting_func(backward=True):
            ...
            total_loss = loss(body_model_output, camera=camera,
                              gt_joints=gt_joints, p3DGT_hand=p3DGT_hand, psmplx_bodyGT=psmplx_bodyGT,
...
class SMPLifyLoss(nn.Module):
    ...
    def __init__(self, search_tree=None,
        ...
        self.register_buffer('data_init_prior_weight',
                             torch.tensor(data_init_prior_weight, dtype=dtype))
    ...
    def forward(self, body_model_output, camera, gt_joints, p3DGT_hand, psmplx_bodyGT, joints_conf,
                body_model_faces, joint_weights,
                use_vposer=False, vposer=None, pose_embedding=None,
                **kwargs):
        pred_vert = body_model_output.vertices.float()
        new_replace_vert = torch.matmul(torch.from_numpy(regressor_mat).cuda().float(), pred_vert[0])
        pred_joints = body_model_output.joints.float()
        index_list = [10]
        pred_joints[0][18] = (pred_joints[0][18] + new_replace_vert[index_list]) / 2.
        projected_joints = get_2dkps_float(body_model_output.joints.float()[0], camera.float())
        projected_joints = projected_joints[None]

        p3dgt_joints = torch.zeros((gt_joints.shape[0], gt_joints.shape[1], 3)).cuda()
        p3dgt_joints[0][91:] = p3DGT_hand

        pred_joints = pred_joints[:, src2inter]
        projected_joints = projected_joints[:, src2inter]
        gt_joints = gt_joints[:, dst2inter]
        p3dgt_joints = p3dgt_joints[:, dst2inter]
        hand_index = [i for i in range(12, 42)] + [i for i in range(53, 63)]
        ...
        relative_pred_hand_3d = (pred_hand_3d - pred_hand_3d[:, 0:1])
        relative_gt_hand_3d = (gt_hand_3d - gt_hand_3d[:, 0:1])
        relative_pred_hand_3d = normalize_points_torch(relative_pred_hand_3d[0])[None]
        relative_gt_hand_3d = normalize_points_torch(relative_gt_hand_3d[0])[None]

        joint_hand_3d_diff = self.robustifier(((relative_pred_hand_3d) - (relative_gt_hand_3d)))
        ...
        if use_vposer:
            ...
            pprior_loss += 1200 * torch.abs(
                vposer.decode(pose_embedding, output_type='aa').view(1, -1)[:, 0:11*3] - psmplx_bodyGT[:,0:11*3]).sum() + \
                self.data_init_prior_weight * torch.abs(vposer.decode(pose_embedding, output_type='aa').view(1, -1)[:, 11*3:] - psmplx_bodyGT[:, 11*3:]).sum()
