...
class EBodyPoseRefiner_sub(nn.Module):
    def __init__(self,
                 total_bones=24,
                 embedding_size=69,
                 mlp_width=256,
                 mlp_depth=4,
                 **_):
        super(EBodyPoseRefiner_sub, self).__init__()
...
class EBodyPoseRefiner(nn.Module):
    def __init__(self,
                 total_bones=24,
                 embedding_size=69,
                 mlp_width=256,
                 mlp_depth=4,
                 **_):
        super(EBodyPoseRefiner, self).__init__()

        self.total_bones = total_bones - 1

        body_bones = 22
        self.body_refiner = EBodyPoseRefiner_sub(total_bones=body_bones, embedding_size=3*(body_bones-1), mlp_width=128, mlp_depth=2)
        face_bones = 4
        self.face_refiner = EBodyPoseRefiner_sub(total_bones=face_bones, embedding_size=3*(face_bones-1), mlp_width=128, mlp_depth=2)
        hand_bones = 31
        self.hand_refiner = EBodyPoseRefiner_sub(total_bones=hand_bones, embedding_size=3*(hand_bones-1), mlp_width=128, mlp_depth=2)

        self.rodriguez = RodriguesModule()

    def forward(self, pose_input):
        rvec_body = self.body_refiner(pose_input[:, :63])
        rvec_face = self.face_refiner(pose_input[:, 63:72])
        rvec_hand = self.hand_refiner(pose_input[:, 72:])
        rvec = torch.cat([rvec_body, rvec_face, rvec_hand])
