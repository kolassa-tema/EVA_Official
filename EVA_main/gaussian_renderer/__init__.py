...
def render(viewpoint_camera, pc : GaussianModel, pipe, bg_color : torch.Tensor, scaling_modifier = 1.0,
    ...
    # pc.motion_offset_flag = False  # only for debug
    ...
    rendered_image_bf, depth, alpha, radii = rasterizer(
        means3D = means3D,
        means2D = means2D,
        shs = shs,
    ...
    if rendered_image is None:
        rendered_image = rendered_image_bf

    conf = pc.refine_net(torch.cat([rendered_image_bf[None], depth[None]], dim=1))[0]

    # Those Gaussians that were frustum culled or had a radius of 0 were not visible.
    # They will be excluded from value updates used in the splitting criteria.
    return {'render_bf': rendered_image_bf,
